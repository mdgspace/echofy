    ...
    <div className="">
      <ul>
        {messages.map((message, index) => (
          <li key={index} className={`flex items-start ${message.isSent ? "justify-end" : "justify-start"} mb-4 mx-4 `}>
            <div className={`relative flex ${message.isSent ? "flex-row-reverse" : ""}`}>
              ...
              <div
                className={`rounded-lg shadow-[0px_4px_4px_rgba(0,0,0,0.25)] max-sm:w-[60vw] max-md:w-[50vw] max-lg:w-[40vw] max-2xl:w-[40vw] 2xl:max-w-[40vw] 2xl:min-w-[5vw] px-4 py-2 m-2 ${
              ...
              >
                ...
                <div className={`text-xs text-white ${message.isSent ? "text-white" : "text-black"} mt-2`}>{formatTime(message.timestamp)}</div>
              </div>
            </div>
          </li>
        ))}
