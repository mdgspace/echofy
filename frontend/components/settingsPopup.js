...
import { MdMusicNote, MdNotifications, MdClose , MdAudiotrack } from 'react-icons/md';


const SettingsPopup = ({ onClose , soundEnabled, setSoundEnabled, notificationsEnabled, setNotificationsEnabled }) => {
        ...
        <div className="p-6 rounded-2xl shadow-2xl relative w-96 h-96 bg-[#F95131AB] bg-opacity-40">
          <div className='flex justify-between items-center mb-[4vh]'>
            <div className='flex items-center'> 
              <Image src={Settings} alt="Settings" className="h-10 w-10 object-contain" />
            </div>
            <button onClick={onClose} className="text-2xl text-red-600 bg-red-200 rounded-xl aspect-square p-3">
                <MdClose></MdClose>
                </button>
          </div>
